    ...
    for col in ['騎手', '厩舎', '馬主']:
        for name, group in df.groupby(['場名', col] if col=='騎手' else col):
            ...
            for i in range(len(rows)-1):
                ...
                if pats:
                    is_c = any(x in pats for x in ['C','D','G','H']) # チャンスパターン
                    for r_data in [r1, r2]:
        ...
        for idx, row in df.iterrows():
            ...
            for _, p_row in prev_match.iterrows():
                if {row['正番'],row['逆番'],row['正循環'],row['逆循環']}.intersection({p_row['正番'],p_row['逆番'],p_row['正循環'],p_row['逆循環']}):
                    df.at[idx, 'タイプ_list'].append('★前日同配置')
                    df.at[idx, '属性_list'].append(f'前日:騎手:{row["騎手"]}')
                    df.at[idx, 'パターン_list'].append('前日')
                    df.at[idx, 'スコア'] += 8.3
...
with st.sidebar:
    ...
    st.divider()
    # CSVダウンロードボタン（重要：ここから保存してAI学習データにします）
    if 'analyzed_df' in st.session_state:
        st.header("💾 経過を保存")
...
if up_curr:
    ...
    if status == "success":
        ...
        # ① 結果入力（修正済み：全頭表示して着順入力可能に）
        st.subheader("📝 結果入力 (3着以内のみ入力でOK)")
        st.info("※負けた馬は空欄のままで構いません（保存時に自動処理されます）")
            ...
            for f_tab, pl in zip(f_tabs, sorted(future_df['場名'].unique())):
                with f_tab:
                    ...
                    for fr_tab, r in zip(fr_tabs, sorted(p_f['R'].unique())):
                        with fr_tab: 
                            st.dataframe(
                                p_f[p_f['R'] == r].sort_values('総合スコア', ascending=False)[['正番','馬名','単ｵｯｽﾞ','属性','エネルギー状態','総合スコア','推奨買い目']], 
                                use_container_width=True, 
                                hide_index=True
                            )
        ...
        if not df_res.empty:
            ...
            for s_tab, s_pl in zip(s_tabs, ["合計"] + sorted(full_df['場名'].unique())):
                with s_tab:
                    ...
                    df_fk = df_s[df_s['着順'] <= 3] # 3着以内を的中とみなす
                    ...
                    if df_s.empty:
                        ...
                        with c_m:
                            # 消化レース数計算（Rの数）
                            finished_races = df_s[['場名', 'R']].drop_duplicates()
                            st.metric("消化R", len(finished_races))
                            
                            hit_rate = len(df_fk)/len(df_s)*100 if len(df_s)>0 else 0
                            st.metric("複勝率", f"{hit_rate:.1f}%")
                            st.metric("的中", f"{len(df_fk)}頭")
                        ...
                        with c_c:
                            ...
                            if all_p:
                                fig = px.pie(
                                    pd.Series(all_p).value_counts().reset_index(), 
                                    values='count', 
                                    names='index', 
                                    title=f'【{s_pl}】的中パターン', 
                                    hole=0.4
